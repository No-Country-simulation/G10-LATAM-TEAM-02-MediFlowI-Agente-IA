...
const DocumentDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState("extracted"); // 'extracted' | 'ocr'
  ...
  const fetchDocument = async () => {
    ...
    try {
      ...
      setError(err.message || `No se pudo encontrar el documento ${id}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="document-detail-page">
      ...
      <div className="d-flex flex-wrap align-items-center justify-content-between mb-4">
        <div className="d-flex align-items-center">
          <Link to="/documentos" className="btn btn-outline-secondary btn-sm me-3 rounded-pill px-3">
            <FaArrowLeft className="me-1" /> Volver al historial
          </Link>
          <div>
            <h3 className="fw-bold text-dark mb-0 d-flex align-items-center">
              Resultado del Triaje <span className="text-primary ms-2">({doc.documento_id})</span>
            </h3>
            <span className="text-muted small">Recepción: {formatDate(doc.fecha_creacion)}</span>
          </div>
        </div>
        ...
        <div className="d-flex gap-2 align-items-center mt-2 mt-md-0">
          ...
          {requiereAuditoria && (
            <Link to={`/auditoria/${doc.documento_id}`} className="btn btn-warning text-dark fw-bold btn-sm ms-2 rounded-pill px-3 shadow-sm">
              <FaUserCheck className="me-1" /> Ir a Auditoría Humana
            </Link>
          )}
        </div>
      </div>
            ...
            <div className="card-header bg-white border-bottom py-3 px-3">
              <div className="row align-items-center">
                ...
                <div className="col-md-5 text-md-end">
                  ...
                  <span className="text-muted small">Tipo: <strong className="text-dark">{doc.clasificacion?.tipo_documento || "Documento Clínico"}</strong></span>
                </div>
              </div>
            </div>
