...
def test_obtener_configuracion(client, auth_headers):
...
def test_actualizar_configuracion_local(client, auth_headers):
    """POST /api/v1/settings permite cambiar a modo LOCAL."""
    response = client.post("/api/v1/settings", headers=auth_headers, json={"storage_mode": "LOCAL"})
    assert response.status_code == 200
    data = response.json()
    assert data["storage_mode"] == "LOCAL"


def test_actualizar_configuracion_oci_o_validacion(client, auth_headers):
    """POST /api/v1/settings procesa la solicitud de OCI según credenciales."""
    response = client.post("/api/v1/settings", headers=auth_headers, json={"storage_mode": "OCI"})
