...
Revision ID: k4l404288gh0
Revises: j3k303277fg9
...
UPGRADE_STATEMENTS = [
    ...
    """DO $$ BEGIN
        IF EXISTS (SELECT 1 FROM auditorias_hitl WHERE auditor_id IS NOT NULL AND auditor_id !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$') THEN
    ...
    """DO $$ BEGIN
        IF EXISTS (SELECT 1 FROM cola_procesamiento WHERE asignado_a IS NOT NULL AND asignado_a !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$') THEN
    ...
    """CREATE TABLE IF NOT EXISTS historial_documento (
        ...
        estado_anterior VARCHAR(50), estado_nuevo VARCHAR(50), descripcion TEXT,
...
def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS historial_documento;")
    op.execute("ALTER TABLE cola_procesamiento ADD COLUMN IF NOT EXISTS asignado_a VARCHAR(255);")
    op.execute("UPDATE cola_procesamiento SET asignado_a = asignado_a_usuario_id::TEXT WHERE asignado_a_usuario_id IS NOT NULL;")
    op.execute("ALTER TABLE cola_procesamiento DROP COLUMN IF EXISTS asignado_a_usuario_id;")
    op.execute("ALTER TABLE auditorias_hitl DROP CONSTRAINT IF EXISTS fk_ahitl_auditor_usuario;")
    op.execute("ALTER TABLE auditorias_hitl ALTER COLUMN auditor_id TYPE VARCHAR(255) USING auditor_id::TEXT;")
    op.execute("ALTER TABLE documentos_triaje DROP COLUMN IF EXISTS usuario_registro_id;")
    op.execute("ALTER TABLE documentos_triaje ALTER COLUMN status SET DEFAULT 'pendiente_auditoria';")
    op.execute("ALTER TABLE cola_procesamiento ALTER COLUMN status SET DEFAULT 'pendiente_auditoria';")
